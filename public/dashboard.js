/**
 * SISTEMA DE GESTÃO - DASHBOARD JS (v22 - FINAL PREMIUM)
 * - Correção total de alinhamento em todas as tabelas.
 * - Classes col-actions aplicadas para evitar quebra de linha.
 * - Correção de funções duplicadas de exclusão.
 */
...
// --- CONFIGURAÇÃO VISUAL (ALERTAS PADRÃO) ---
...
const Alert = Swal.mixin({
    ...
    confirmButtonColor: '#d4af37', // Dourado Padrão
...
function setupStaticListeners() {
    ...
    if(btnLogout) btnLogout.addEventListener('click', async (e) => {
        e.preventDefault(); await fetch(API.logout); window.location.href = 'login.html';
    });
    ...
    if(document.getElementById('btnConfirmarNovaRemessa')) {
        document.getElementById('btnConfirmarNovaRemessa').addEventListener('click', function() {
            ...
            .then(res => {
                if (res.success) {
                    document.getElementById('modalNovaRemessa').classList.remove('open');
...
window.abrirModalNovaRemessa = function() {
    ...
    fetch(API.fornecedores).then(r => r.json()).then(data => {
        ...
        document.getElementById('modalNovaRemessa').classList.add('open');
    }).catch(e => {
...
window.solicitarExclusao = function(id, tipo, idsArray = null) {
    ...
    if (tipo === 'lote') {
        ...
        desc.innerText = `Você selecionou ${idsArray.length} itens para remover do lote.`;
    } else if (tipo === 'produto_massa') {
        titulo.innerText = 'Excluir Produtos?';
        desc.innerText = `Você vai apagar ${idsArray.length} produtos do estoque.`;
    } else {
...
window.confirmarExclusaoReal = async function() {
    ...
    try {
        if (tipo === 'lote') {
            ...
            await fetch(API.lote_itens + '?ids=' + ids.join(','), { method: 'DELETE' });
...
window.venderProduto = function(id) {
    vendaPendenteId = id; 
    document.getElementById('modalConfirmarVenda').classList.add('open'); 
};
...
window.confirmarVendaReal = async function() {
    ...
    try {
        ...
        document.getElementById('modalConfirmarVenda').classList.remove('open');
...
window.salvarEdicaoVenda = async function() {
    ...
    try {
        ...
        if((await res.json()).success) {
            document.getElementById('modalEditarVenda').classList.remove('open');
...
window.abrirModal = function(tipo, id=null, nome=null, parent=null, extra=null) {
    ...
    document.getElementById('modalTitulo').innerText = titles[tipo] || 'Item';
    document.getElementById('modalGenerico').classList.add('open');
    window.modalAction = tipo;
    setTimeout(() => document.getElementById('modalInput').focus(), 100);
};

window.solicitarExclusao = function(id, tipo) {
...
function carregarLotes() {
    ...
    fetch(API.lotes)
        ...
        .then(lotes => {
            grid.innerHTML = '';
            if (lotes.length === 0) {
                grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 50px; color: #999; border: 2px dashed #eee; border-radius: 20px;"><p>Nenhuma remessa encontrada.</p><button class="btn btn-primary" onclick="window.abrirModalNovaRemessa()" style="margin-top:10px;">Criar Primeira</button></div>`;
                return;
            }
            lotes.forEach(lote => {
...
window.abrirLote = function(id, status) {
    ...
    if(status === 'ativo') {
        ...
        badge.style.background = '#e6fffa'; badge.style.color = '#00b894';
    ...
    // Se Ativo (Aprovado): Mostra Relatório
        ...
        toolbar.innerHTML = `
            <button class="btn-tool" onclick="window.abrirConferenciaLote(${id})" style="background:#00b894;"><i class="ph-bold ph-check-circle"></i> Conferir / Devolver</button>
        `;
        toolbar.style.display = 'flex';
    }
...
window.atualizarItemLote = async function(id, campo, valor, inputElement) {
    ...
    if((campo === 'preco_custo' || campo === 'preco_venda') && (!valor || valor < 0)) return;
...
function carregarItensDoLote(id) {
    ...
    const isLocked = (window.loteAtualStatus === 'ativo' || window.loteAtualStatus === 'finalizado');
    ...
    const inputStyleBase = "width:100%; border:1px solid transparent; background:transparent; padding:8px; border-radius:6px; font-size:13px; color:#444;";
    const inputStyle = isLocked ? `${inputStyleBase} opacity: 0.8; cursor: not-allowed;` : `${inputStyleBase} transition:0.2s;`;
    const codeStyle = `${inputStyle} font-family:'Courier New', monospace; font-weight:700; letter-spacing:-0.5px; background:#f9f9f9;`;

    tbody.innerHTML = '<tr><td colspan="7" class="center"><div class="spinner"></div></td></tr>';
    
    fetch(API.lote_itens + `?lote_id=${id}&t=${new Date().getTime()}`)
        .then(r => r.json())
        .then(data => {
            tbody.innerHTML = '';
            ...
            if(data.itens && data.itens.length > 0) {
                data.itens.forEach(item => {
                    ...
                    let c = parseFloat(item.preco_custo) || 0;
                    let v = parseFloat(item.preco_venda) || 0;
                    totalQtd += q; totalCusto += c * q; totalVenda += v * q;
                });
            } else {
                tbody.innerHTML = '<tr><td colspan="7" class="center" style="padding:40px; color:#999;">Nenhum item adicionado. Use o Scanner ou Manual.</td></tr>';
            }

            document.getElementById('loteQtd').innerText = totalQtd;
            document.getElementById('loteCusto').innerText = formatMoney(totalCusto);
            document.getElementById('loteVenda').innerText = formatMoney(totalVenda);
            document.getElementById('loteLucro').innerText = formatMoney(totalVenda - totalCusto);
            ...
            if(data.itens) {
                data.itens.forEach(item => {
                    ...
                    // --- HTML DO CONTROLE DE QUANTIDADE PERSONALIZADO ---
                    const qtdControl = `
                    ...
                    tbody.innerHTML += `
                        <tr>
                            <td class="center" style="display:${displayCheckbox}; width:50px;">
                                <input type="checkbox" class="check-lote" value="${item.id}" onchange="window.atualizarBarraAcoesLote()" ${disabledAttr}>
                            </td>
                            <td><input type="text" value="${item.codigo_produto}" onchange="window.atualizarItemLote(${item.id}, 'codigo_produto', this.value, this)" ${disabledAttr} style="${codeStyle}"></td>
                            <td><input type="text" value="${item.nome}" onchange="window.atualizarItemLote(${item.id}, 'nome', this.value, this)" ${disabledAttr} style="${inputStyle}"></td>
                            <td><select onchange="window.atualizarItemLote(${item.id}, 'categoria_id', this.value, this)" ${disabledAttr} style="${inputStyle}">${optionsComSelect}</select></td>
                            
                            <td style="width:120px;">${isLocked ? `<input value="${item.quantidade}" disabled style="text-align:center; border:none; background:transparent;">` : qtdControl}</td>

                            <td><input type="number" step="0.01" value="${item.preco_custo}" onchange="window.atualizarItemLote(${item.id}, 'preco_custo', this.value, this)" ${disabledAttr} style="${inputStyle}"></td>
                            <td><input type="number" step="0.01" value="${item.preco_venda}" onchange="window.atualizarItemLote(${item.id}, 'preco_venda', this.value, this)" ${disabledAttr} style="${inputStyle} color:var(--success); font-weight:600;"></td>
                        </tr>
                    `;
                });
            }
...
async function abrirConferenciaLote(loteId) {
    ...
    try {
        ...
        let htmlItens = `
        <div class="rel-digital-container">
            <div class="rel-header">
                <div>
                    <h3>Relatório de Fechamento</h3>
                    <span>Lote #${data.lote.id} • ${new Date().toLocaleDateString()}</span>
                </div>
                <div style="text-align:right;">
                    <strong style="display:block; font-size:18px; color:#2d3436;">R$ ${data.resumo.total_vendido_valor.toFixed(2)}</strong>
                    <span style="font-size:11px; text-transform:uppercase;">A Pagar</span>
                </div>
            </div>
        ...
        data.itens.forEach(i => {
            let statusBadge = i.vendido > 0 
                ? '<span class="rel-status st-vendido">Vendido</span>' 
                : '<span class="rel-status st-estoque">Estoque</span>';
            
            htmlItens += `
                <div class="rel-row">
                    <div style="flex:1;">
                        <span class="rel-item-name">${i.nome}</span>
                        <span class="rel-item-sub">${i.codigo}</span>
                    </div>
                    <div style="text-align:right;">
                        ${statusBadge}<br>
                        <span style="font-weight:600; font-size:12px;">R$ ${parseFloat(i.total_pagar).toFixed(2)}</span>
                    </div>
                </div>
            `;
        });
        ...
        htmlItens += `</div>
            <div class="rel-total-box">
                <div>
                    <small style="display:block; color:#aaa;">Devolução</small>
                    <strong style="font-size:16px;">${data.resumo.qtd_devolver} un</strong>
                </div>
                <div style="text-align:right;">
                    <small style="display:block; color:#aaa;">Vendas</small>
                    <strong style="font-size:16px;">${data.resumo.qtd_vendida} un</strong>
                </div>
            </div>
        </div>`;

        const isFinalizado = data.lote.status === 'finalizado';
        
        // --- MODAL DO RELATÓRIO (USANDO O NOVO ESTILO) ---
        Swal.fire({
            html: htmlItens,
            width: '450px',
            padding: '0',
            ...
            confirmButtonText: isFinalizado ? 'OK' : '✅ Finalizar', 
            confirmButtonColor: isFinalizado ? '#2d3436' : '#00b894',
...
async function realizarBaixaDevolucao(loteId) {
    const confirm = await Swal.fire({
        ...
        text: "Os itens devolvidos sairão do sistema e o lote será fechado.",
        icon: 'warning',
        showCancelButton: true,
    ...
    try {
        ...
        if (json.success) {
            Swal.fire('Sucesso!', 'Lote finalizado e estoque atualizado.', 'success');
            carregarLotes(); 
            carregarProdutos(); 
        } else {
