...
function renderProdutos(lista) {
    ...
    grid.innerHTML = lista.map((p, index) => {
        let imgUrl = 'https://via.placeholder.com/400x400?text=Sem+Foto';
        if (p.imagem) {
            if (p.imagem.includes('uploads/')) {
                imgUrl = p.imagem; 
            } else {
                imgUrl = CAMINHO_IMAGEM + p.imagem;
            }
        }
...
function enviarWhatsApp() {
    ...
    if(!cleanWhats) return alert("Esta loja não configurou um número de WhatsApp.");
